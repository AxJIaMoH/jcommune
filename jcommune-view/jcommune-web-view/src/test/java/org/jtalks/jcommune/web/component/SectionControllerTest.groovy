...
@Transactional
class SectionControllerTest extends Specification {
    ...
    def 'hello world test'() {
        given: 'hello world test method'
        when: 'hello world test method will be called'
        then: 'everything become good'
    }

    def 'test create topic'() {
        given: 'Branch created, user signed in and has permissions to create topics in current branch'
            def branch = Branches.create()
            Users.createAndSignIn().withPermissionOn(branch, BranchPermission.CREATE_POSTS);
        when: 'User creates topic'
        def result = mockMvc.perform(post("/topics/new")
